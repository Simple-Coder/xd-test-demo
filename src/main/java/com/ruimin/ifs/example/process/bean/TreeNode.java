...
public class TreeNode {
	...
	private String toHTML(int deep, boolean checkboxs) {
		...
		if (!this.children.isEmpty() || this.hasChild) {
			sb.append("<span class=\"tree-hit ").append("closed".equals(this.state) || this.hasChild ? "tree-collapsed" : "tree-expanded").append("\" />");
		} else {
			sb.append("<span class=\"tree-indent\" />");
		}
		sb.append("<span class=\"tree-icon " + (this.children.isEmpty() && !this.hasChild ? "tree-file" : "tree-folder") + " ").append("closed".equals(this.state) || this.hasChild ? "" : "tree-folder-open").append(this.iconCls).append("\" />");
		if (checkboxs) {
