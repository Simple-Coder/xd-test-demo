...
import com.ruimin.ifs.login.process.bean.UserExtInfo;
...
public class LoginService extends SnowService {
	...
	public SessionUserInfo loginUserSessionInfo(String username, String password, String brhno) throws SnowException {
		...
		if (StringUtils.isBlank(brhno)) {
			...
			if (bctl == null) {
				SnowExceptionUtil.throwErrorException(SnowErrorCode.MESSAGE_SYS_0005, new String[]{"机构信息["+staff.getBrcode()+"]不存在！"});
			}
			userinfo.setBrno(bctl.getBrno());
		}else{
			userinfo.setBrno(brhno);
			List<Object> orgList = DBDaos.newInstance().selectList("com.ruimin.ifs.login.rql.login.queryOrgByNo",RqlParam.map().set("brno",brhno));
			if (orgList.size()>0) {
		...
		// 扩展信息
		UserExtInfo extInf = new UserExtInfo();
		IfsStaffExt tblStaffExt = this.getTblStaffExt(username);
		if (tblStaffExt != null) {
			if (StringUtils.isNotBlank(tblStaffExt.getHeadImg())) {
				extInf.setHeadImg(true);
			}
			extInf.setEmail(tblStaffExt.getEmail());
			extInf.setTelNo(tblStaffExt.getTelNo());
		}
		userinfo.getExtenMap().put(SnowConstant.USER_EXT_INF_KEY, extInf);
		return userinfo;
	}
	 ...
	 */
	public IfsStaffExt getTblStaffExt(String trlNo) throws SnowException {
		DBDao dao = DBDaos.newInstance();
		IfsStaffExt tlrInfo = dao.select(IfsStaffExt.class, trlNo);
		return tlrInfo;
	}
	
	public String getSysParamDef(String paramId, String magicId, String defaultVal) {
		DBDao dao = DBDaos.newInstance();
		TblSysParam param = (TblSysParam) dao.selectOne("com.ruimin.ifs.login.rql.login.getSystemParamByKey", RqlParam
				.map().set("key1", paramId).set("key2", magicId));
		if (param == null) {
	...
	public boolean comparePassword(String sInputPassword, String sRightPassword, String sEncMethod)
		...
		if (sEncMethod == null || sEncMethod.equalsIgnoreCase("") || sEncMethod.equalsIgnoreCase("none")) {
			...
			sTransPassword = EncryptUtil.getEncrypt(EncryptType.valueOf(sEncMethod.toUpperCase())).encrypt(sInputPassword);
		}
		return sTransPassword.equals(sRightPassword);
	}

	private Map<String, Function> getUserFuncMap(TblStaff staff) throws SnowException {
		Map<String, Function> map = new LinkedHashMap<String, Function>();
		DBDao dao = DBDaos.newInstance();
		// 根据角色获取权限
	 ...
	 * @throws SnowException
	 */
	public TblStaff checkUser(String username, String password, String brhno) throws SnowException {
		DBDao dao = DBDaos.newInstance();
		TblStaff tlrInfo = (TblStaff) dao.selectOne("com.ruimin.ifs.login.rql.login.queryUserByUsernameAndPassword",
		...
		if (tlrInfo == null) {
			...
			if(StringUtils.isNotBlank(brhno)){
				List<Object> orgList = dao.selectList("com.ruimin.ifs.login.rql.login.queryOrgByNo",RqlParam.map().set("brno",brhno));
				if (orgList.size() <= 0) {
	...
	private void checkPassword(TblStaff user, String password) throws SnowException {
		...
		{
//			System.out.println("");
			log.info("当前账户：【{}】,密码：【{}】已是MD5加密后的算法，通过custLogin.jsp传入，校验通过",user.getTlrno(),password);
			return;
		}
		...
		if (SnowConstants.LOCKED.equals(user.getIsLock())) {
			if (lockingTime < 0) {
				...
				if (between < lockingTime || lockingTime < 0) {
					SnowExceptionUtil.throwErrorException(SnowErrorCode.MESSAGE_WEB_0022, new String[] { lockingTime
							+ "" });
				} else {
		...
		//等于默认密码强制修改
		String defPwd = SysParamUtil.getInstance().getString("PSWD.DEFAULT_PWD", "123456");
		if(comparePassword(defPwd, user.getPassword(), enc)){
	...
	public TblStaff checkUserPwd(String userLoginId, String password, String newPasswd) throws SnowException {
		...
		//密码不能为默认密码
		String defPwd = SysParamUtil.getInstance().getString("PSWD.DEFAULT_PWD", "123456");
		if (newPasswd.trim().equals(defPwd)) {
			SnowExceptionUtil.throwWarnException(SnowConstant.WEB_ERROR_CODE_E0045);
		}
		DBDao dao = DBDaos.newInstance();
	...
	public void updateUserLogOut(){
		DBDaos.newInstance().executeUpdate("com.ruimin.ifs.login.rql.login.updateStaffStatusLogOut", RqlParam.map().set("lastLogOutTm", DateUtil.getDateAndTm14()));
	}
	 ...
	 * @throws SnowException
	 */
	public void updateStaffByPageSet(String email,String telNo,String imgStr) throws SnowException{
		DBDao dao = DBDaos.newInstance();
		SessionUserInfo user = SessionUserInfo.getSessionUserInfo();
		IfsStaffExt tlrInfo = dao.select(IfsStaffExt.class, user.getTlrno());
		
		if (tlrInfo!=null) {
			if (StringUtils.isNotBlank(imgStr)) {
				tlrInfo.setHeadImg(imgStr);
			}
			tlrInfo.setTelNo(telNo);
			tlrInfo.setEmail(email);
			dao.update(tlrInfo);
		}else{
			tlrInfo = new IfsStaffExt();
			SessionUserInfo sessionUserInfo = SessionUserInfo.getSessionUserInfo();
			tlrInfo.setTlrno(sessionUserInfo.getTlrno());
			if (StringUtils.isNotBlank(imgStr)) {
				tlrInfo.setHeadImg(imgStr);
			}
			tlrInfo.setTelNo(telNo);
		...
		UserExtInfo staffExt = (UserExtInfo) user.getExtenMap().get(SnowConstant.USER_EXT_INF_KEY);
		if (StringUtils.isNotBlank(imgStr)) {
			staffExt.setHeadImg(true);
		}
		staffExt.setEmail(email);
		staffExt.setTelNo(telNo);
	}
}
