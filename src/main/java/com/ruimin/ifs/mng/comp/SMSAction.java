...
import com.ruimin.ifs.mng.process.service.SMSService;
import com.ruimin.ifs.po.TblBeBankReturnCode;
import com.ruimin.ifs.po.TblBeSms;
...
@ActionResource
@Slf4j
public class SMSAction extends SnowAction {// 继承SnowAction
    ...
    public PageResult queryBeReturnCode(QueryParamBean queryBean) throws SnowException {
        ...
        String querySmsType = queryBean.getParameter("querySmsType");
        // 还可以使用 getParameter(String key,String defaultValue)来设置空值的默认值
        String queryStatus = queryBean.getParameter("queryStatus");

        log.info("短信查询，传入参数querySmsType：" + querySmsType + "；queryStatus:" + queryStatus);

        // 调用服务层
        PageResult result = SMSService.getInstance().querySMSList(querySmsType, queryStatus, queryBean.getPage());
        if (result != null && result.getTotalCount() > 0) {
            List<TblBeSms> list = (List<TblBeSms>) result.getQueryResult();
            for (TblBeSms sms : list) {
                sms.setCreateDate(StringUtils.isEmpty(sms.getCreateDate()) ? "" : DateUtil.convertStringToTimeString(sms.getCreateDate()));
                sms.setLastUpdateTime(StringUtils.isEmpty(sms.getLastUpdateTime()) ? "" : DateUtil.convertStringToTimeString(sms.getLastUpdateTime()));
            }
        }
        log.info("短信查询结束");
