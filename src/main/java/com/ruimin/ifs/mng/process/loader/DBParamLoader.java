...
public class DBParamLoader implements IParamLoader {
	...
	public Map<String, String> get(String group) {
		if (isCluster() || !GlobalUtil.isProductMode()) {
			...
			List list = dao.selectList("com.ruimin.ifs.mng.rql.sysmng.querySysParamByGroup", RqlParam.map().set("group", group));
	...
	public String get(String group, String key) {
		...
		if (isCluster() || !GlobalUtil.isProductMode()) {
			DBDao dao = DBDaos.newInstance();
			List list = dao
			        .selectList("com.ruimin.ifs.mng.rql.sysmng.querySysParamByGroupAndKey", RqlParam.map().set("group", group).set("key", key));
			if (!list.isEmpty()) {
