...
@Service
public class BeBankReturnCodeService extends SnowService {// 继承自SnowService

    /**
     * @return 服务实例，单例
     * @throws SnowException
     */
    public static BeBankReturnCodeService getInstance() throws SnowException {
        // 根据class获取服务实例
        return ContextUtil.getSingleService(BeBankReturnCodeService.class);
    }

    /**
     * 接入银行返回码模糊查询
     *
     * @param queryBankCode     银行代码
     * @param queryBeReturnCode 银企平台返回码
     * @param page              分页信息
     * @return 交易定义管理查询结果
     * @throws SnowException
     */
    public PageResult queryReturnCodeList(String queryBankCode, String queryBeReturnCode, Page page)
            throws SnowException {
        // 获取一个DAO对象，它会当前默认的数据源
        DBDao dao = DBDaos.newInstance();
        // 调用查询方法，key为RQL配置文件的包名+<select>节点的id
        return dao.selectList(
                "com.ruimin.ifs.mng.rql.BeBankReturnCode.queryReturnCodeList",
                RqlParam.map().set("queryBankCode", StringUtils.isEmpty(queryBankCode) ? "" : queryBankCode).set("queryBeReturnCode", StringUtils.isEmpty(queryBeReturnCode) ? "" : queryBeReturnCode), page);
    }
    ...
    public int queryBeReturnCode(String bankCode, String beReturnCode, String returnCode) throws SnowException {
        DBDao dao = DBDaos.newInstance();
        return (Integer) dao.selectOne("com.ruimin.ifs.mng.rql.BeBankReturnCode.queryBeReturnCode", RqlParam.map().set("bankCode", bankCode).set("beReturnCode", beReturnCode).set("returnCode", returnCode));
    }
