...
public class BiNationregionEntryService extends SnowService {
	...
	public PageResult queryList(String qid, String qnationregionNumber, String cnEnFullName, String cnEnShortName, Page page) throws SnowException{
		DBDao dao = DBDaos.newInstance();
		return dao.selectList("com.ruimin.ifs.mng.rql.sysmng.queryBiNationregionEntry", 
			RqlParam.map()
				.set("qid", qid==null?"":"%"+qid.toUpperCase()+"%")
				.set("qnationregionNumber", qnationregionNumber==null?"":"%"+qnationregionNumber+"%")
				.set("cnEnFullName", cnEnFullName==null?"":"%"+cnEnFullName+"%")
				.set("cnEnShortName", cnEnShortName==null?"":"%"+cnEnShortName.toUpperCase()+"%"),
				page);
	}
