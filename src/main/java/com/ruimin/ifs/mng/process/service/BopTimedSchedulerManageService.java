...
public class BopTimedSchedulerManageService extends SnowService {
	...
	public List<TblCronJobVO> queryList() throws SnowException{
		...
		for (int i = 0; i < list.size(); i++) {
			...
			newList.add(cronJob);
			if (StringUtils.isNotBlank(cronJob.getProcessParam())) {
				Map<String, String> jobParamToMap = CronJobUtil.jobParamToMap(cronJob.getProcessParam());
				boolean transaction = CronJobUtil.isTransaction(jobParamToMap);
				cronJob.setTransactionFlg(String.valueOf(transaction));
			}else {
				cronJob.setTransactionFlg("true");
			}
		}
		return newList;
	}
	 ...
	 */
	public PageResult queryListLog(String excuteTimeStart, String excuteTimeEnd, String excuteResult, Page page) throws SnowException{
		DBDao dao = DBDaos.newInstance();
		return dao.selectList("com.ruimin.ifs.mng.rql.sysmng.queryJobLog", 
			RqlParam.map()
				.set("excuteTimeStart", excuteTimeStart==null?"":excuteTimeStart)
