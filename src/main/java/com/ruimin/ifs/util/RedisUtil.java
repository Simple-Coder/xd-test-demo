...
public class RedisUtil {
    private static final RedisUtil redisUtilsSecond = new RedisUtil();
    ...
    static {
        prop = new Properties();
        try {
            prop.load(new FileInputStream(new File(RedisUtil.class.getClassLoader().
                    getResource("genconf/spring/redis.properties").getPath())));
            //
            JedisPoolConfig config = new JedisPoolConfig();
            //最大空闲接连
            config.setMaxIdle(Integer.parseInt(prop.getProperty("spring.redis.pool.max-idle")));
            //最小空闲连接
            config.setMinIdle(Integer.parseInt(prop.getProperty("spring.redis.pool.min-idle")));
            //连接池最大阻塞等待时间
            config.setMaxWaitMillis(Integer.parseInt(prop.getProperty("spring.redis.pool.max-wait")));
            //在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
            config.setTestOnBorrow(true);
            //无密码配置的redis
//            jedisPool = new JedisPool(config, prop.getProperty("spring.redis.host"), Integer.parseInt(prop.getProperty("spring.redis.port")));
            //有密码设置的redis
            jedisPool = new JedisPool(config,
                    prop.getProperty("spring.redis.host"),
                    Integer.parseInt(prop.getProperty("spring.redis.port")),
                    Integer.parseInt(prop.getProperty("spring.redis.timeout")),
                    prop.getProperty("spring.redis.password"));
        } catch (IOException e) {
            log.error("redis连接池出现问题", e);
        }
    }
