...
function converDateSetParameter2Map(dataset, paramMap) {
	...
	//20180810 增加tab ticket及funid处理
	paramMap["_tab_ticket"] = _getTabCurrentTicket();
	paramMap["_tab_itemid"] = _getTabCurrentFuncId();
	return paramMap;
}

//20180810 增加tab ticket及funid处理
function _getTabCurrentTicket(){
	if(typeof(top._getTabSelectedByTicketValue) == "function"){
		return top._getTabSelectedByTicketValue();
	}
	return "";
}
function _getTabCurrentFuncId(){
	if(typeof(top._getTabSelectedByItemId) == "function"){
		return top._getTabSelectedByItemId();
	}
	return "";
}
