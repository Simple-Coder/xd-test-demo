...
//	UIUtil.loadFile("/public/lib/fontawesome/css/font-awesome.css");
	UIUtil.loadFile("/public/lib/fontawesome4/css/font-awesome.css");
	initCallGetter();
}
...
function record_getString(fieldName) {
	...
	try {
		if ((field.tag == "selectName" || field.tag == "radioName") && value == "") {
			if (field.datasource.indexOf("DDIC") == 0 || field.datasource.indexOf("LIST") == 0) {
				var fieldid = field.fieldName.substring(0, field.fieldName.length - 4);
				result = UIUtil.DDIC(record.dataset.id + "_" + fieldid, record.getValue(fieldid))
			}
		}
	} catch (e) {
	} finally {
...
function _record_setValue(record, fieldName, value) {
	...
	if (dataset.loadCompleted) {
		var event_result = fireDatasetEvent(dataset, "beforeChange", [ dataset, field, value, record ]);
		if (event_result === false) {
	...
	fireDatasetEvent(dataset, "afterChange", [ dataset, field, value, record ]);
	// }
...
function converDateSetParameter2Map(dataset, param) {
	...
	parameters["_tab_ticket"] = _getTabCurrentTicket();
	parameters["_tab_itemid"] = _getTabCurrentFuncId();
