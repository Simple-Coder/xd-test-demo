...
(function(g, $, undefined) {
	...
	function ExportTabSet(tabs) {
		...
		function _refresh(id, url) {
			tabs.reload(id, url ? util.webpath(url) : url);
		}
		function _length() {
	...
	$.extend(ui.event.onRefresh, {
		...
		combogrid : function(jq, dataset) {
			...
			if (combobox.grid && jq.data("tmpds") == dataset.id) {
				var json = dataset.toJson();
				var source = {
					localdata : [],
					datatype : "array",
			...
			render : function(jq, options) {
				...
				options = $.extend({
					...
					onAfterSelectTabItem : function(tabid, oldtabid) {
						...
						function triggerTagChange(tabid, oldtabid, iframe) {
							var eventName = util.join(jq.attr("id"), "afterTabChange");
							if (window[eventName]) {
								setTimeout(function() {
									window[eventName](tabid, oldtabid, iframe);
								}, 1);
							}
						}
						if (iframeholder.length) {
							...
							iframe.bind('load.tab', function() {
								...
								triggerTagChange(tabid, oldtabid, this);
							});// .bind('focus.tab', triggerFocus);
							iframeholder.remove();
						} else {
							triggerFocus();
							if (tabid != oldtabid) {
								triggerTagChange(tabid, oldtabid, $("iframe", contentitem)[0]);
							}
						}
					}
				});
	...
	function translateDataset2Bean(button) {
		...
		//20180810 增加tab ticket及funid处理 start
  		mutilBeanMap["_tab_ticket"] = _getTabCurrentTicket();
  		mutilBeanMap["_tab_itemid"] = _getTabCurrentFuncId();
  		//20180810 增加tab ticket及funid处理 end
		var multiMap = {
			dsName : btnCqId,
			dsPath : btnDSPath,
			// updCqId : btnCqId,
	...
	function _button_click(button) {
		...
		if (button.defaultOperation) {
			...
			case "asyncqrysubmitreset":
				...
				if (state != 'none') {
					for (var j = 0; j < dataset.fields.fieldCount; j++) {
						fireDatasetEvent(dataset, "afterChange", [ dataset, dataset.getField(j) ]);
					}
				}
	...
	function _initColumn(options, gridid, ds, fid, c) {
		...
		if (options.fitcolumns === false) {
			column.minWidth = Math.min(80, c || 80);
		}
			...
			render : function(jq, options) {
				...
				for (var i = isselectfirst ? 1 : 0; i < fieldids.length; i++) {
					...
					if (hd1idx > -1) {// 第一层
						...
						var c = unC(fieldid);
						column = _initColumn(options, gridid, ds, c[0], c[1]);
					}
					columns.push(column);
				}
				...
				options = $.extend({
					...
					fixedCellHeight : false,
					// rownumbers : true,
					emptyMsg : i18n.grid && i18n.grid.empty,
				...
				if (!init) {
					...
					allEl.bind("click", function(e) {
						if(this.checked){
							...
							var maxValue = endEl.val();
							var maxpage = Math.ceil(dataset.expmaxrcd * 1.0 / dataset.pageSize);
							if(dataset.pageCount>maxpage){
								maxValue = maxpage;
								var message = util.format(i18n.exportMaxRecord, dataset.expmaxrcd);
								tipEl.html(message);
							}else{
								maxValue = dataset.pageCount;
								tipEl.html("");
							}
							endEl.manager().set({
								disabled : true,
