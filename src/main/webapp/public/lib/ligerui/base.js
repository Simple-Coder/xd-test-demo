...
(function($) {
	...
	window.liger = $.ligerui = {
		...
		add : function(manager) {
			...
			// if (this.managers[manager.id]) manager.id =
			// this.getId(manager.__idPrev());
	...
	$.extend(liger.core.Component.prototype, {
		...
		disableEvent : function(eventName) {
			...
			this.disabledEvent[name] = true;
		},
		enableEvent : function(eventName) {
			var name = eventName.toLowerCase();
			this.disabledEvent[name] = false;
		},
	...
	liger.controls.Input.ligerExtend(liger.core.UIComponent, {
		...
		_setRequired : function(required) {
			...
			if (!wrapper || !wrapper.hasClass("l-text"))
				return;
			var rightTag = wrapper.find(".l-text-r");
			if (rightTag[0]) {
				if (required) {
					rightTag.addClass("l-required").html("*");
				} else {
					rightTag.removeClass("l-required").empty();
				}
			}
		},
		...
		setValid : function(bol) {
			this.invalid = !bol;
			this.trigger("valid", [ bol ]);
		}
	});
	...
	liger.win = {
		...
		mask : function(win) {
			...
			this.windowMask.css("z-index", $(win.element).css("z-index")-1);
		...
		// 取消遮罩
		unmask : function(win) {
			var jwins = $("body > .l-dialog:visible,body > .l-window:visible").filter("[ligeruiid!="+win.id+"]");
			for (var i = 0, l = jwins.length; i < l; i++) {
				...
				this.windowMask.css("z-index", jwins.last().css("z-index")-1);
				// 如果存在其他模态窗口，那么不会取消遮罩
				if (modal)
					return;
			}
