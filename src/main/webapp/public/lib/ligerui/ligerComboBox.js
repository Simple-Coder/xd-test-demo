...
(function($) {
	...
	$.ligerDefaults.ComboBox = {
		...
		onButtonClick : null, // 右侧图标按钮事件，可以通过return
		// false来阻止继续操作，利用这个参数可以用来调用其他函数，比如打开一个新窗口来选择值
	...
	$.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
		...
		_render : function() {
			...
			if (this.element.tagName.toLowerCase() == "input") {
				...
				g.textFieldID = this.element.name;
			...
			if (g.inputText[0].name == undefined) {
				...
				g.inputText[0].name = g.textFieldID + "_txt";
			}
			// 隐藏域初始化
			g.valueField = null;
			if (p.valueFieldID) {
			...
			if (p.triggerIcon)
				g.link.find("div:first").addClass(p.triggerIcon);
			// 下拉框
...
//			g.wrapper.append(g.link);
			...
			g.link.hover(function() {
				...
				// return false;
				var show = g.selectBox.is(":visible");
				if (!show) {
					if (g.trigger('beforeOpen') == false)
						return false;
				}
				// zzg end
				g._toggleSelectBox(show);
			});
			...
			// zzg end
//			g.updateSelectBoxPosition(); 展开后再计算位置
		...
		clear : function() {
			...
			// this.trigger('clear'); //zzg del
		},
		clearBox : function(ids) {// zzg added
			$("a.l-checkbox-checked", this.selectBox).removeClass("l-checkbox-checked");
			$("tr.l-selected", this.selectBox).removeClass("l-selected");
			$(":checkbox", this.selectBox).each(function() {
				this.checked = false;
			});
		},
		_setSelectBoxHeight : function(height) {
			...
			if (p.grid) {
				...
				$(".l-box-select-inner", g.selectBox).height(height);
			}
		},
		...
		_setValue : function(value, t) {
			var g = this, p = this.options;
			var text = g.findTextByValue(value) || t;
...
//				g._changeValue(value, text);
				$("table.l-table-checkbox :checked", g.selectBox).prop("checked", false);
				$("table.l-table-checkbox .l-checkbox-checked", g.selectBox).removeClass("l-checkbox-checked");
				value += "";
				if (value) {
					var targetdata = value.toString().split(p.split);
					for (var i = 0; i < targetdata.length; i++) {
						...
						$("table.l-table-checkbox tr[value=" + targetdata[i] + "] a.l-checkbox", g.selectBox).each(function() {
		...
		selectValue : function(value, text) {// zzg
		...
		setSelect : function() {
			...
			$('option', g.select).each(function(i) {
				var val = $(this).val();
				var txt = $(this).html();
				var tr = $("<tr><td index='" + i + "' value='" + val + "' text='" + txt + "'>" + txt + "</td></tr>");
		...
		setTree : function(tree) {
			var g = this, p = this.options;
			p.hideOnLoseFocus = p.hideTreeOnLoseFocus ? true : false;// zzg
			this.clearContent();
			g.selectBox.table.remove();
			// zzg begin
			if (tree.checkbox != false) {
				tree.onCheck = function(node, checked) {
					...
					var children = $(".l-children > li", node.target);
					var records = [ node.data ];
					children.each(function() {
						var treedataindex = parseInt($(this).attr("treedataindex"));
						var data = g.treeManager._getDataNodeByTreeDataIndex(g.treeManager.data, treedataindex);
						if (data) {
							records.push(data);
						}
					});

					function getParentStateCascade(treeitem, fn) {
						var parent = $(treeitem).parent();
						var parentitem = parent.parent("li");
						// 当前同级别或低级别的节点是否都选中了
						var isCheckedComplete = $(".l-checkbox-unchecked", parent).length == 0;
						// 当前同级别或低级别的节点是否都没有选中
						var isCheckedNull = $(".l-checkbox-checked", parent).length == 0;
						if (isCheckedComplete) {
							parent.prev().find(".l-checkbox").removeClass("l-checkbox-unchecked l-checkbox-incomplete").addClass("l-checkbox-checked");
							fn(parentitem);
						} else if (isCheckedNull) {
							parent.prev().find("> .l-checkbox").removeClass("l-checkbox-checked l-checkbox-incomplete").addClass("l-checkbox-unchecked");
							fn(parentitem);
						} else {
							parent.prev().find("> .l-checkbox").removeClass("l-checkbox-unchecked l-checkbox-checked").addClass("l-checkbox-incomplete");
						}
						if (parentitem.length > 0)
							getParentStateCascade(parentitem, fn);
					}

					getParentStateCascade(node.target, function(item) {
					...
					g.trigger("treeselect", [ checked, records ]);
				};
			} else {
				tree.onSelect = function(node) {
					if (g.trigger('BeforeSelect'[node]) == false)
						return;
					if (node.data["_canSelected"] === false || node.data["_canSelected"] === "false")
						return;
					// var value = node.data[p.valueField];
					// var text = node.data[p.textField];
					// g._changeValue(value, text);
					g.trigger("treeselect", [ true, [ node.data ] ]);
					g._toggleSelectBox(true);
				};
			...
			tree.onRemoteLoad = function(param, success, error) {
				g.trigger("remoteLoad", [ param, success, error ]);
			};
		...
		setGrid : function(grid) {
			...
			if (p.condition) {
				var conditionParm = $.extend({
					labelWidth : p.selectBoxWidth - 240,
					space : 20,
			...
			if (grid.checkbox) {
				g.grid.bind('CheckAllRow', function(checked, panelElement) {
					var records = [];
					for ( var p in g.grid.records) {
						records.push(g.grid.records[p]);
					}
					g.trigger("gridselect", [ checked, records ]);
				});
				g.grid.bind('CheckRow', function(checked, rowdata, oid, tr) {
					g.trigger("gridselect", [ checked, rowdata, oid, tr ]);
				});
			} else {
				g.grid.bind('clickRow', function(rowdata, oid, tr) {
					g._toggleSelectBox(true);
					g.trigger("gridselect", [ true, [ rowdata ], oid, tr ]);
				});
			}
...
//				});
		...
		updateSelectBoxPosition : function() {
			...
			if (p && p.absolute) {
				...
				if (p.alwayShowInTop || Number(g.wrapper.offset().top + 1 + g.wrapper.outerHeight() + g.selectBox.height()) > contentHeight && contentHeight > Number(g.selectBox.height() + 1)) {
					// 若下拉框大小超过当前document下边框,且当前document上留白大于下拉内容高度,下拉内容向上展现
					g.selectBox.css({
						left : g.wrapper.offset().left,
						top : g.wrapper.offset().top - 1 - g.selectBox.height()
					});
				} else {
					g.selectBox.css({
						left : g.wrapper.offset().left,
				...
				if (p.alwayShowInTop || Number(g.wrapper.offset().left + g.selectBox.width()) > contentWidth) {
					g.selectBox.css({
						left : g.wrapper.offset().left + g.wrapper.outerWidth() - g.selectBox.width()
					});
				}
			} else {
