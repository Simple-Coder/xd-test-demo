...
(function($) {
	...
	$.ligerDefaults.Grid = {
		...
		autoFilter : false, // 自动生成高级查询, 需要filter/toolbar组件支持.
		emptyMsg : '',
		// 需要引用skins/ligerui-icons.css
	...
	$.ligerDefaults.Grid_columns = {
		...
		render : null,
		textField : null
	// 真正显示的字段名,如果设置了，在编辑状态时,会调用创建编辑器的setText和getText方法
	};
	...
	$.ligerDefaults.Grid.formatters['number'] = function(value, column) {
		return formatFloat(value, column.scale);
	}
	$.ligerDefaults.Grid.formatters['currency'] = function(value, column) {
		return "$" + formatCurrency(formatFloat(value, column.scale));
	}
	// zzg end
	...
	$.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
		...
		_init : function() {
			...
			for ( var type in UIKit.editors) {
				p.editors[type] = UIKit.grid.getEditor({
					type : type,
					master : g
				});
			}
		...
		_render : function() {
			...
			var gridhtmlarr = [];
			gridhtmlarr.push("        <div class='l-panel-header'><span class='l-panel-header-text'></span>");
			if (p.collapsible) {
				gridhtmlarr.push("       <div class='icons'><div class='l-panel-header-toggle'></div></div>");
			}
			...
			gridhtmlarr.push("                </div>");
			gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'> <input type='text' size='10' value='1' class='l-bar-btngoto' maxlength='10' /> / <span></span></span></div>");
			gridhtmlarr.push("                <div class='l-bar-separator'></div>");
			gridhtmlarr.push("                <div class='l-bar-group'>");
			...
			if (p.exportbar) {
				var exportstr = "|" + p.exportbar.toLowerCase() + "|";
				var exptypes = [ "csv", "xls", "pdf", "doc", "html" ];
				gridhtmlarr.push("                <div class='l-bar-separator'></div>");
				gridhtmlarr.push("                <div class='l-bar-group'>");
				for (var i = 0, len = exptypes.length; i < len; i++) {
					var exptype = exptypes[i];
					if (exportstr.indexOf("|" + exptype + "|") > -1) {
						gridhtmlarr.push("                    <div class='l-bar-button l-bar-export l-bar-export-" + exptype + "'><span></span></div>");
					}
				}
				gridhtmlarr.push("                </div>");
			}
			...
			if (p.paginationbar) {
				gridhtmlarr.push("            <div class='l-paginationbar'></div>");
			}
			...
			g.paginationbar = $(".l-paginationbar", g.toolbar);
		...
		_calculateGridBodyHeight : function(h) {
			...
			if (p.title)
				h -= g.header.outerHeight();// 24
			if (p.usePager)
				h -= g.toolbar.outerHeight();// 32
			if (p.totalRender)
				h -= 25;
			if (p.toolbar)
				h -= g.topbar.outerHeight();
			// var gridHeaderHeight = p.headerRowHeight * (g._columnMaxLevel -
			// 1) + p.headerRowHeight;
			h -= g.gridheader.outerHeight();// gridHeaderHeight;
			return h;
		},
		...
		_createEditor : function(editorBuilder, container, editParm, width, height) {
			...
			// var editor = editorBuilder.create.call(this, container,
			// editParm);
		...
		beginEdit : function(rowParm, containerBulider) {
			var g = this, p = this.options;
			if (!p.enabledEdit || p.clickToEdit || g.editor.invalid)
				return;
...
//			g.isDataChanged = true;
		...
		setColumnWidth : function(columnparm, newwidth) {
			...
			$(cells).css('width', newwidth).find("> div.l-grid-row-cell-inner:first").css('width', newwidth - 0 );
		...
		changeHeaderText : function(columnparm, headerText) {
			...
			var columnindex = column['__leafindex'];
			var headercell = document.getElementById(column['__domid']);
			$(".l-grid-hd-cell-text", headercell).html(headerText).attr("title", headerText);
		...
		_initBuildHeader : function() {
			...
			if (p.toolbar) {
				if (typeof p.toolbar == "string") {
					g.topbar.append($("#" + p.toolbar));

				} else if ($.fn.ligerToolBar) {
		...
		_createHeaderCell : function(column) {
			...
			// var jcell = $("<td class='l-grid-hd-cell'><div class='l-grid-hd-cell-inner'><span class='l-grid-hd-cell-text'></span><span class='l-grid-hd-cell-filter '>&nbsp;</span></div></td>");
			...
			if (column.headAlign) {
				$(".l-grid-hd-cell-inner:first", jcell).css("textAlign", column.headAlign);
			}
			if (column['__colSpan'])
			...
			if (column.display && column.display != "")
				...
				headerText = "&nbsp;";
			$(".l-grid-hd-cell-text:first", jcell).html(headerText).attr("title", headerText);
		...
		_initFootbar : function() {
			...
			if (p.usePager) {
				...
				// if (p.paginationbar && p.paginationbar.length) {
				// g._setPaginationbar(p.paginationbar);
				// }
			} else {
				g.toolbar.hide();
			}
		},
		_setPaginationbar : function(buttons) {
			if (buttons) {
				var items = [];
				for (var i = 0, len = buttons.length; i < len; i++) {
					...
					if (button.id == "-") {
						...
						if (window[button.id]) {
							...
							window[button.id].allwaysHidden = true;
							items.push(item);
						}
					}

				}
				this.paginationbar.ligerToolBar({
					items : items
				});
			}
		},
		...
		_fillGridBody : function(data, frozen) {
			...
			if (frozen)
				gridhtmlarr.push('<div class="l-jplace"></div>');
		...
		_showData : function() {
			...
			if (p.usePager) {
				// 更新总记录数
				if (p.dataAction == "server" && g.data && (g.data[p.record] || g.data[p.record] === 0))
			...
			if (!data || !data.length) {
				...
				$("<div class='empty-data'>"+p.emptyMsg+"</div>").addClass("l-grid-body-inner").appendTo(g.gridbody).css({
					width : g.gridheader.find(">div:first>table").width(),
		...
		_fixHeight : function() {
			...
			for ( var rowid in g.records) {
				...
				var h1 = $(cell1).height(), h2 = $(cell2).height();
				var height = Math.max(h1, h2, 25);
				$(cell1).add(cell2).height(height);
				if (firstrow) {//当有换行单元格时，rownumber高度对不齐
					firstrow = false;
					var cell1h = $(cell1).height(), cell2h = $(cell2).height();
					if (cell1h < height && cell1h != cell2h) {
						$(cell1).height(height + 1);
					}
//					if (cell2h < height) {
//						$(cell2).height(height + 1);
//					}
				}
...
//							gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
		...
		_getCellHtml : function(rowdata, column) {
			...
			// htmlarr.push('<div');
			htmlarr.push(' style = "width:' + parseInt(column._width - 0) + 'px;');
			if (p.fixedCellHeight)
		...
		_applyEditor : function(obj) {
			...
			if (column.editor.type && p.editors[column.editor.type]) {
				...
				var editor = p.editors[column.editor.type], jcell = $(rowcell), offset = $(rowcell).offset(), width = $(rowcell).width(), height = $(rowcell).height(), container = $("<div class='l-grid-editor'></div>").appendTo(g.gridbody.find(".l-grid-body-inner:first")), left = 0, top = 0, pc = jcell.position(), pb = g.gridbody
						.position(), pv = g.gridview2.position(),
				// 加上括号解决不能正常判定topBar的高度。不加括号会忽略后面运算出来的值
				topbarHeight = (p.toolbar ? g.topbar.parent().outerHeight() : 0) + (p.title ? g.header.outerHeight() : 0);//, left = pc.left + pb.left + pv.left, top = pc.top + pb.top + pv.top + topbarHeight;
				left = pc.left + g.gridbody.scrollLeft(), top = pc.top + g.gridbody.scrollTop() + 2;

				//jcell.html("");优化
				jcell.find(".l-grid-row-cell-inner").css("visibility","hidden");
				g.setCellEditing(rowdata, column, true);
				container.css({
					left : left,
					top : ($.browser.safari ? top : top - 1) + p.editorTopDiff,
					width : width,
					height : height,
					display : "table"
				});//.show();
				if (column.textField) {
					if (column.getText) {
						editParm.text = column.getText(currentdata, column.textField);
					} else {
						editParm.text = g._getValueByName(rowdata, column.textField);
					}
				}
				...
				g.editors[rowdata['__id']] = g.editors[rowdata['__id']] || {};
				g.editors[rowdata['__id']][column['__id']] = {
					editor : editor,
					input : editorInput,
					editParm : editParm,
				...
				g.bind('sysEndEdit', function() {
					...
					if (newValue != currentdata) {
						var ischg = true;//优化
						if (column.editor.type == 'datebox') {
							if ($.type(newValue)=='date') {
								if (newValue.format(UIKit.properties.dateFormat) == currentdata) {
									ischg = false;
								}
							}
						} else if (column.editor.type == 'datetimebox') {
							if ($.type(newValue)=='date') {
								if (newValue.format(UIKit.properties.datetimeFormat) == currentdata) {
									ischg = false;
								}
							}
						} else {
		...
		_getSrcElementByEvent : function(e) {
			...
			if (r.toolbar) {
				...
				if (fn("l-bar-export-xls")) {
					r.exporter = "xls";
				} else if (fn("l-bar-export-pdf")) {
					r.exporter = "pdf";
				} else if (fn("l-bar-export-csv")) {
					r.exporter = "csv";
				}
			}
		...
		_setEvent : function() {
			...
			$('select', g.toolbar).change(function() {
				if (g.isDataChanged && p.dataAction != "local")
					return false;
				p.newPage = 1;
				p.pageSize = this.value;
				g.trigger("changePageSize", [ this.value ]);
		...
		_onClick : function(e) {
			...
			} else if (src.hcelltext) // 排序
			{
				...
				if (src.column.isSort == false)
					return;
				if (p.url && p.dataAction != "local" && g.isDataChanged)
					return;
			...
			} else if (src.row && g.enabledCheckbox()) // 复选框选择行
			{
				...
				if (src.checkbox || !selectRowButtonOnly) {
					...
					var rowdatas = [ src.data ];
					if (p.tree && p.autoCheckChildren) {
						var children = g.getChildren(src.data, true);
						for (var i = 0, l = children.length; i < l; i++) {
							g[met](children[i]);
							rowdatas.push(children[i]);
						}
					}

					g.trigger('checkRow', [ !uncheck, rowdatas ]);
				}
				if (!src.checkbox && src.cell && (p.enabledEdit && !g.editor.invalid) && p.clickToEdit) {
					g._applyEditor(src.cell);
				}
			} else if (src.row && !g.enabledCheckbox()) {
				if (src.cell && (p.enabledEdit && !g.editor.invalid) && p.clickToEdit) {
					g._applyEditor(src.cell);
				}
				...
				if (src.first) {
					...
					if (p.url && g.isDataChanged)
						return false;
					g.loadData(p.where);
				} else if (src.exporter) {
					if (window[p.exporter]) {
						window[p.exporter].show(src.exporter);
					}
				}
			}
		},
		select : function(rowParm) {
			...
			if (g.enabledCheckbox()) {
				var selectedtrs = $(".l-grid-row.l-selected", g.f.gridbody).length;
				var selectedtrs2 = $(".l-grid-row", g.f.gridbody).length;
				if (selectedtrs == selectedtrs2) {
					$(".l-grid-hd-row", g.gridview1).addClass("l-checked");
				}
			}
		},
		unselect : function(rowParm) {
			...
			if (g.enabledCheckbox()) {
				$(".l-grid-hd-row", g.gridview1).removeClass("l-checked");
			}
		},
		isSelected : function(rowParm) {
			var g = this, p = this.options;
			if (rowParm && (rowParm.select === true || rowParm.select === "true")) {
				return true;
			}
			var rowdata = g.getRow(rowParm);
			for ( var i in g.selected) {
