...
(function($) {
	...
	$.ligerDefaults.Portal = {
		width : null,
		/*
		 * 行元素：组件允许以纵向方式分割为几块 每一块(行)允许自定义N个列(column) 每一列允许自定义N个Panel(最小元素)
		 * rows:[ {columns:[ { width : '50%', panels :
		 * [{width:'100%',content:'内容'},{width:'100%',url:@url1}] },{ width :
		 * '50%', panels : [{width:'100%',url:@url2}] } ]} ]
		 */
		rows : null,
		/*
		 * 列元素： 组件将认为只存在一个row(块), 这一块 允许自定义N个列(column),结构同上
		 */
		columns : null,
	...
	$.ligerui.controls.Portal.ligerExtend($.ligerui.core.UIComponent, {
		...
		_init : function() {
			...
			if ($(">div", g.element).length) // 如果已经定义了DIV子元素,那么这些元素将会转换为columns,这里暂时保存到tempInitPanels
			{
				...
				$(">div", g.element).each(function(i, jpanel) {
					var option = UIUtil.parseOptions(this);
					var column = $.extend({
						panels : []
					}, option);
					p.columns[i] = column;
					...
					$(">div", this).each(function() {
						...
						if (panel.url) {
							panel.content = "<iframe id='" + this.id + "_iframe' class='inner-iframe' frameborder='0' src='" + UIUtil.webpath(option.url) + "'></iframe>";
							panel.url = null;
							delete panel.url;
						}
						p.columns[i].panels.push(panel);
					});
		...
		_renderColumn : function(e) {
			...
			if (column.panels && column.panels.length) {
				for (var i = 0; i < column.panels.length; i++) {
					...
					var jpanel = $('<div></div>').attr("id", panel.id).appendTo(jcolumn);
		...
		setPanelEvent : function(e) {
			...
			var g = this, p = this.options;
			// 拖拽支持
			if ($.fn.ligerDrag && (p.draggable && panel.options.draggable)) {
				jpanel.addClass("l-panel-draggable").ligerDrag({
					...
					onDrag : function(current, e) {
						...
						var centerX = offset.left + 20 / 2, centerY = offset.top + 10;
						...
						if (result) {
							...
							if (this.placeStatus) {
								if (this.placeStatus.panel && result.panel) {
									if (this.placeStatus.panel.rowIndex == result.panel.rowIndex && this.placeStatus.panel.columnIndex == result.panel.columnIndex && this.placeStatus.panel.index == result.panel.index && this.placeStatus.position == result.position) {
										return;
									}
								}
