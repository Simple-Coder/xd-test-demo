...
(function($) {
	...
	$.ligerui.controls.RadioList.ligerExtend($.ligerui.controls.Input, {
		...
		indexOf : function(item) {
			...
			for (var i = 0, l = g.data.length; i < l; i++) {
				if (typeof (item) == "object") {
					...
					if (g.data[i][p.valueField].toString() == item.toString())
						return i;
				}
			}
			return -1;
		...
		_dataInit : function(value) {
			var g = this, p = this.options;
			//var value = value || g.valueField.val() || g._getValue() || p.value;
		...
		_addClickEven : function() {
			var g = this, p = this.options;
			// 选项点击
			g.radioList.delegate(":radio", "click", function(e) {
				var value = g.getValue();
				// zzg begin
				// if (value)
				// g.valueField.val(value);
				g.trigger("click", [ value ]);
				// zzg end
			});
		}
	});
